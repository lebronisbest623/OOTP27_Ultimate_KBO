...
static int kbo_write_bytes(uint8_t* address, const void* value, SIZE_T size)
{
    if (address == NULL || value == NULL || size == 0 || !memory_range_readable(address, size)) {
        return 0;
    }
...
{
    return kbo_write_bytes(address, &value, sizeof(value));
}

int kbo_write_u8(uint8_t* address, uint8_t value)
{
    return kbo_write_bytes(address, &value, sizeof(value));
}
