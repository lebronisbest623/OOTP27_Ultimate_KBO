    ...
    uint32_t source_rva)
{
    ...
    static LONG prepare_hook_log_count = 0;
    LONG slot = InterlockedIncrement(&prepare_log_count);
    int should_log = slot <= 120;
    if (!should_log && source_rva != 0u) {
        LONG hook_slot = InterlockedIncrement(&prepare_hook_log_count);
        should_log = hook_slot <= 240;
    }
    if (should_log) {
        kbo_log_runtimef(
