#include "../ui_fa_views_internal.h"

static const char* kbo_fa_compensation_subview_label(int subview)
{
    switch (subview) {
    case KBO_HUB_FA_COMP_SUBVIEW_TASKS:      return "보상 업무";
    case KBO_HUB_FA_COMP_SUBVIEW_PROTECTION: return "보호명단";
    case KBO_HUB_FA_COMP_SUBVIEW_DECISION:   return "보상 결정";
    case KBO_HUB_FA_COMP_SUBVIEW_HISTORY:    return "기록";
    default:                                 return "";
    }
}

static void kbo_webview_append_fa_compensation_subtabs(KboWindowTextBuffer* buffer, int selected_subview)
{
    kbo_window_text_appendf(buffer, "<nav class='faCompTabs'>");
    for (int i = 0; i < KBO_HUB_FA_COMP_SUBVIEW_COUNT; i++) {
        kbo_window_text_appendf(
            buffer,
            "<a class='faCompTab %s' href='kbo://fa-comp/view/%d'>",
            i == selected_subview ? "active" : "",
            i);
        kbo_html_append_escaped(buffer, kbo_fa_compensation_subview_label(i));
        kbo_window_text_appendf(buffer, "</a>");
    }
    kbo_window_text_appendf(buffer, "</nav>");
}

static void kbo_webview_append_fa_compensation_text_action(
    KboWindowTextBuffer* buffer,
    int enabled,
    const char* class_name,
    const char* title,
    const char* href,
    const char* label)
{
    if (enabled) {
        kbo_window_text_appendf(buffer, "<a class='rightsTextAction");
        if (class_name != NULL && class_name[0] != '\0') {
            kbo_window_text_appendf(buffer, " ");
            kbo_html_append_escaped(buffer, class_name);
        }
        kbo_window_text_appendf(buffer, "'");
        if (title != NULL && title[0] != '\0') {
            kbo_window_text_appendf(buffer, " title='");
            kbo_html_append_escaped(buffer, title);
            kbo_window_text_appendf(buffer, "'");
        }
        kbo_window_text_appendf(buffer, " href='");
        kbo_html_append_escaped(buffer, href);
        kbo_window_text_appendf(buffer, "'>");
        kbo_html_append_escaped(buffer, label);
        kbo_window_text_appendf(buffer, "</a>");
    } else {
        kbo_window_text_appendf(buffer, "<span class='rightsTextAction disabled");
        if (class_name != NULL && class_name[0] != '\0') {
            kbo_window_text_appendf(buffer, " ");
            kbo_html_append_escaped(buffer, class_name);
        }
        kbo_window_text_appendf(buffer, "' title='");
        kbo_html_append_escaped(buffer, title != NULL && title[0] != '\0' ? title : "내가 맡은 구단이 아닙니다");
        kbo_window_text_appendf(buffer, "'>");
        kbo_html_append_escaped(buffer, label);
        kbo_window_text_appendf(buffer, "</span>");
    }
}

static int kbo_fa_compensation_needs_protected_list(const KboFaCompensationRecord* rec)
{
    if (rec == NULL || !rec->requires_player_compensation || rec->protect_count == 0u) {
        return 0;
    }
    return rec->status == KBO_FA_COMPENSATION_STATUS_PENDING
        || rec->status == KBO_FA_COMPENSATION_STATUS_RECORDED;
}

static int kbo_fa_compensation_decision_ready(
    const KboFaCompensationRecord* rec,
    const KboFaCompensationProtectionDebugRow* debug_rows,
    int debug_count)
{
    return rec != NULL
        && rec->requires_player_compensation
        && rec->protect_count > 0u
        && !kbo_fa_compensation_record_is_final(rec)
        && kbo_fa_compensation_debug_row_count(debug_rows, debug_count, rec->player_id) > 0;
}

static KboFaCompensationRecord* kbo_fa_compensation_find_detail_record(
    KboFaCompensationRecord* records,
    int record_count,
    const KboFaCompensationProtectionDebugRow* debug_rows,
    int debug_count,
    uint32_t selected_player_id)
{
    if (selected_player_id != 0u) {
        for (int i = 0; i < record_count; i++) {
            if (records[i].player_id == selected_player_id
                    && records[i].requires_player_compensation
                    && records[i].protect_count > 0u) {
                return &records[i];
            }
        }
    }
    for (int i = 0; i < record_count; i++) {
        if (kbo_fa_compensation_decision_ready(&records[i], debug_rows, debug_count)) {
            return &records[i];
        }
    }
    for (int i = 0; i < record_count; i++) {
        if (kbo_fa_compensation_needs_protected_list(&records[i])) {
            return &records[i];
        }
    }
    for (int i = 0; i < record_count; i++) {
        if (records[i].requires_player_compensation && records[i].protect_count > 0u) {
            return &records[i];
        }
    }
    return NULL;
}

static void kbo_fa_compensation_copy_record_text(
    const KboFaCompensationRecord* rec,
    char* original_team,
    size_t original_team_size,
    char* signing_team,
    size_t signing_team_size,
    char* previous_salary,
    size_t previous_salary_size,
    char* cash_with_player,
    size_t cash_with_player_size,
    char* cash_only,
    size_t cash_only_size)
{
    if (rec == NULL) {
        return;
    }
    kbo_hub_copy_team_abbrev_by_id(rec->original_team_id, original_team, original_team_size, "-");
    kbo_hub_copy_team_abbrev_by_id(rec->signing_team_id, signing_team, signing_team_size, "-");
    kbo_fa_market_format_salary(rec->previous_salary, previous_salary, previous_salary_size);
    kbo_fa_market_format_salary((int32_t)rec->cash_with_player, cash_with_player, cash_with_player_size);
    kbo_fa_market_format_salary((int32_t)rec->cash_only, cash_only, cash_only_size);
}

static void kbo_webview_append_fa_compensation_task_row(
    KboWindowTextBuffer* buffer,
    KboFaCompensationRecord* rec,
    KboFaCompensationRecord* detail_rec,
    const char* task,
    const char* status,
    uint32_t action_team_id,
    const char* action_source,
    const char* href,
    const char* label)
{
    char original_team[16] = "-";
    char signing_team[16] = "-";
    char previous_salary[32] = "-";
    char cash_with_player[32] = "-";
    char cash_only[32] = "-";
    const char* grade_display = kbo_fa_market_display_grade(rec->grade);
    uint32_t grade_sort_rank = kbo_fa_market_display_grade_sort_rank(rec->grade);
    int row_selected = detail_rec != NULL && detail_rec->player_id == rec->player_id;

    kbo_fa_compensation_copy_record_text(
        rec,
        original_team,
        sizeof(original_team),
        signing_team,
        sizeof(signing_team),
        previous_salary,
        sizeof(previous_salary),
        cash_with_player,
        sizeof(cash_with_player),
        cash_only,
        sizeof(cash_only));

    kbo_window_text_appendf(buffer, row_selected ? "<tr class='selected'>" : "<tr>");
    kbo_window_text_appendf(buffer, "<td data-sort-value='%u'>%u</td><td>", rec->signed_on_yyyymmdd, rec->signed_on_yyyymmdd);
    kbo_html_append_escaped(buffer, task);
    kbo_window_text_appendf(buffer, "</td>");
    kbo_webview_append_player_name_cell(buffer, rec->player_name, rec->player_id);
    kbo_window_text_appendf(buffer, "<td>");
    kbo_html_append_escaped(buffer, original_team);
    kbo_window_text_appendf(buffer, "</td><td>");
    kbo_html_append_escaped(buffer, signing_team);
    kbo_window_text_appendf(buffer, "</td><td data-sort-value='%u'>", grade_sort_rank);
    kbo_html_append_escaped(buffer, grade_display);
    kbo_window_text_appendf(buffer, "</td><td class='roStatus'>");
    kbo_html_append_escaped(buffer, status);
    kbo_window_text_appendf(buffer, "</td><td class='roAction'>");
    kbo_webview_append_fa_compensation_text_action(
        buffer,
        kbo_hub_ui_team_action_available(action_team_id, action_source),
        "",
        "",
        href,
        label);
    kbo_window_text_appendf(buffer, "</td></tr>");
}

static void kbo_webview_append_fa_compensation_tasks_view(
    KboWindowTextBuffer* buffer,
    KboFaCompensationRecord* records,
    int record_count,
    const KboFaCompensationProtectionDebugRow* debug_rows,
    int debug_count,
    KboFaCompensationRecord* detail_rec)
{
    kbo_window_text_appendf(
        buffer,
        "<section class='tablewrap rosterTableWrap faCompLedgerWrap'><table class='ootpRosterTable faCompTaskTable'><thead><tr>"
        "<th data-sort-type='number'>일자</th>"
        "<th data-sort-type='text'>업무</th>"
        "<th class='roName' data-sort-type='text'>FA</th>"
        "<th data-sort-type='text'>원 소속</th>"
        "<th data-sort-type='text'>계약 구단</th>"
        "<th data-sort-type='number'>등급</th>"
        "<th class='roStatus' data-sort-type='text'>상태</th>"
        "<th class='roAction' data-sort-type='text'>처리</th>"
        "</tr></thead><tbody>");

    int rendered = 0;
    for (int i = 0; i < record_count && rendered < 800; i++) {
        KboFaCompensationRecord* rec = &records[i];
        char href[96] = {0};
        if (kbo_fa_compensation_record_is_final(rec)) {
            continue;
        }
        if (kbo_fa_compensation_needs_protected_list(rec)) {
            snprintf(href, sizeof(href), "kbo://fa-comp/submit/%u", rec->player_id);
            kbo_webview_append_fa_compensation_task_row(
                buffer,
                rec,
                detail_rec,
                "보호명단",
                "계약 구단 제출 대기",
                rec->signing_team_id,
                "hub_protected_list_submit_render",
                href,
                "제출");
            rendered++;
        } else if (kbo_fa_compensation_decision_ready(rec, debug_rows, debug_count)) {
            snprintf(href, sizeof(href), "kbo://fa-comp/detail/%u", rec->player_id);
            kbo_webview_append_fa_compensation_task_row(
                buffer,
                rec,
                detail_rec,
                "보상 결정",
                "원 소속 결정 대기",
                rec->original_team_id,
                "hub_manual_compensation_select_render",
                href,
                "결정");
            rendered++;
        } else if (rec->requires_player_compensation && rec->protect_count > 0u) {
            snprintf(href, sizeof(href), "kbo://fa-comp/detail/%u", rec->player_id);
            kbo_webview_append_fa_compensation_task_row(
                buffer,
                rec,
                detail_rec,
                "명단 확인",
                kbo_fa_compensation_status_label(rec->status),
                rec->original_team_id,
                "hub_manual_compensation_select_render",
                href,
                "열기");
            rendered++;
        }
    }
    if (rendered == 0) {
        kbo_window_text_appendf(buffer, "<tr><td colspan='8' class='roEmptyMessage'>처리할 FA 보상 업무가 없습니다.</td></tr>");
    }
    kbo_window_text_appendf(buffer, "</tbody></table></section>");
}

static void kbo_webview_append_fa_compensation_protection_view(
    KboWindowTextBuffer* buffer,
    KboFaCompensationRecord* records,
    int record_count,
    KboFaCompensationRecord* detail_rec)
{
    kbo_window_text_appendf(
        buffer,
        "<section class='tablewrap rosterTableWrap faCompLedgerWrap'><table class='ootpRosterTable faCompProtectionTable'><thead><tr>"
        "<th data-sort-type='number'>일자</th>"
        "<th class='roName' data-sort-type='text'>FA</th>"
        "<th data-sort-type='text'>계약 구단</th>"
        "<th data-sort-type='text'>원 소속</th>"
        "<th data-sort-type='number'>보호</th>"
        "<th class='roStatus' data-sort-type='text'>상태</th>"
        "<th class='roAction' data-sort-type='text'>처리</th>"
        "</tr></thead><tbody>");

    int rendered = 0;
    for (int i = 0; i < record_count && rendered < 800; i++) {
        KboFaCompensationRecord* rec = &records[i];
        if (!kbo_fa_compensation_needs_protected_list(rec)) {
            continue;
        }
        char original_team[16] = "-";
        char signing_team[16] = "-";
        char previous_salary[32] = "-";
        char cash_with_player[32] = "-";
        char cash_only[32] = "-";
        char href[80] = {0};
        int row_selected = detail_rec != NULL && detail_rec->player_id == rec->player_id;

        kbo_fa_compensation_copy_record_text(
            rec,
            original_team,
            sizeof(original_team),
            signing_team,
            sizeof(signing_team),
            previous_salary,
            sizeof(previous_salary),
            cash_with_player,
            sizeof(cash_with_player),
            cash_only,
            sizeof(cash_only));
        snprintf(href, sizeof(href), "kbo://fa-comp/submit/%u", rec->player_id);

        kbo_window_text_appendf(buffer, row_selected ? "<tr class='selected'>" : "<tr>");
        kbo_window_text_appendf(buffer, "<td data-sort-value='%u'>%u</td>", rec->signed_on_yyyymmdd, rec->signed_on_yyyymmdd);
        kbo_webview_append_player_name_cell(buffer, rec->player_name, rec->player_id);
        kbo_window_text_appendf(buffer, "<td>");
        kbo_html_append_escaped(buffer, signing_team);
        kbo_window_text_appendf(buffer, "</td><td>");
        kbo_html_append_escaped(buffer, original_team);
        kbo_window_text_appendf(
            buffer,
            "</td><td data-sort-value='%u'>%u명</td><td class='roStatus'>",
            rec->protect_count,
            rec->protect_count);
        kbo_html_append_escaped(buffer, "제출 대기");
        kbo_window_text_appendf(buffer, "</td><td class='roAction'>");
        kbo_webview_append_fa_compensation_text_action(
            buffer,
            kbo_hub_ui_team_action_available(rec->signing_team_id, "hub_protected_list_submit_render"),
            "",
            "계약 구단 보호명단 제출",
            href,
            "제출");
        kbo_window_text_appendf(buffer, "</td></tr>");
        rendered++;
    }
    if (rendered == 0) {
        kbo_window_text_appendf(buffer, "<tr><td colspan='7' class='roEmptyMessage'>제출 대기 중인 보호명단이 없습니다.</td></tr>");
    }
    kbo_window_text_appendf(buffer, "</tbody></table></section>");
}

static void kbo_webview_append_fa_compensation_decision_view(
    KboWindowTextBuffer* buffer,
    const KboFaCompensationProtectionDebugRow* debug_rows,
    int debug_count,
    KboFaCompensationRecord* detail_rec)
{
    if (detail_rec == NULL) {
        kbo_window_text_appendf(
            buffer,
            "<section class='tablewrap rosterTableWrap faCompLists'><table class='ootpRosterTable'><tbody>"
            "<tr><td class='roEmptyMessage'>결정할 선수 보상 건이 없습니다.</td></tr>"
            "</tbody></table></section>");
        return;
    }

    KboFaCompensationDecisionRow decision;
    int has_decision = kbo_load_latest_fa_compensation_decision(detail_rec->player_id, &decision);
    int is_final = kbo_fa_compensation_record_is_final(detail_rec);
    int detail_rows = kbo_fa_compensation_debug_row_count(debug_rows, debug_count, detail_rec->player_id);
    int selected_cash = has_decision && strcmp(decision.action, "CASH_ONLY") == 0;

    kbo_window_text_appendf(
        buffer,
        "<section class='tablewrap rosterTableWrap faCompLists'><table class='ootpRosterTable faCompListTable'><thead><tr>"
        "<th class='faCompPool' data-sort-type='text'>구분</th>"
        "<th class='faCompRank' data-sort-type='number'>순위</th>"
        "<th class='roName' data-sort-type='text'>선수</th>"
        "<th class='faCompAge' data-sort-type='number'>나이</th>"
        "<th class='roAction' data-sort-type='text'>결정</th>"
        "</tr></thead><tbody>");

    if (detail_rows > 0 || selected_cash) {
        char href[80] = {0};
        snprintf(href, sizeof(href), "kbo://fa-comp/cash-only/%u", detail_rec->player_id);
        kbo_window_text_appendf(buffer, selected_cash ? "<tr class='selected'>" : "<tr>");
        kbo_window_text_appendf(
            buffer,
            "<td class='faCompPool'>현금</td><td class='faCompRank' data-sort-value='0'>-</td>"
            "<td class='roName'>현금 보상</td><td class='faCompAge' data-sort-value='0'>-</td><td class='roAction'>");
        if (selected_cash) {
            kbo_window_text_appendf(buffer, "<span class='faCompPick'>선택됨</span>");
        } else if (!is_final) {
            kbo_webview_append_fa_compensation_text_action(
                buffer,
                kbo_hub_ui_team_action_available(detail_rec->original_team_id, "hub_cash_only_select_render"),
                "cashOnly",
                "현금 보상 선택",
                href,
                "현금");
        } else {
            kbo_html_append_escaped(buffer, "-");
        }
        kbo_window_text_appendf(buffer, "</td></tr>");
    }

    int rendered = 0;
    static const char* list_order[] = { "auto_protected", "protected", "unprotected" };
    for (int list_index = 0; list_index < 3; list_index++) {
        const char* list_type = list_order[list_index];
        for (int i = 0; i < debug_count; i++) {
            const KboFaCompensationProtectionDebugRow* row = &debug_rows[i];
            if (row->fa_player_id != detail_rec->player_id || strcmp(row->list_type, list_type) != 0) {
                continue;
            }
            const char* label = strcmp(row->list_type, "auto_protected") == 0
                ? "자동 보호" : (strcmp(row->list_type, "protected") == 0 ? "보호" : "선택 가능");
            int selected_player = has_decision
                && strcmp(decision.action, "PLAYER") == 0
                && row->player_id == decision.selected_player_id;

            kbo_window_text_appendf(buffer, selected_player ? "<tr class='selected'>" : "<tr>");
            kbo_window_text_appendf(buffer, "<td class='faCompPool'>");
            kbo_html_append_escaped(buffer, label);
            kbo_window_text_appendf(buffer, "</td><td class='faCompRank' data-sort-value='%u'>%u</td>", row->rank, row->rank);
            kbo_webview_append_player_name_cell(buffer, row->player_name, row->player_id);
            kbo_window_text_appendf(
                buffer,
                "<td class='faCompAge' data-sort-value='%u'>%u</td><td class='roAction'>",
                (uint32_t)row->age,
                (uint32_t)row->age);
            if (selected_player) {
                kbo_window_text_appendf(buffer, "<span class='faCompPick'>선택됨</span>");
            } else if (!is_final && strcmp(row->list_type, "unprotected") == 0) {
                char href[96];
                snprintf(href, sizeof(href), "kbo://fa-comp/select/%u/%u", detail_rec->player_id, row->player_id);
                kbo_webview_append_fa_compensation_text_action(
                    buffer,
                    kbo_hub_ui_team_action_available(detail_rec->original_team_id, "hub_manual_compensation_select_render"),
                    "",
                    "",
                    href,
                    "선택");
            } else {
                kbo_html_append_escaped(buffer, "-");
            }
            kbo_window_text_appendf(buffer, "</td></tr>");
            rendered++;
        }
    }
    if (detail_rows <= 0 && rendered == 0) {
        kbo_window_text_appendf(buffer, "<tr><td colspan='5' class='roEmptyMessage'>보호명단이 아직 제출되지 않았습니다.</td></tr>");
    }
    kbo_window_text_appendf(buffer, "</tbody></table></section>");
}

static void kbo_webview_append_fa_compensation_history_view(
    KboWindowTextBuffer* buffer,
    KboFaCompensationRecord* records,
    int record_count,
    KboFaCompensationRecord* detail_rec)
{
    kbo_window_text_appendf(
        buffer,
        "<section class='tablewrap rosterTableWrap faCompLedgerWrap'><table class='ootpRosterTable faCompTable'><thead><tr>"
        "<th data-sort-type='number'>일자</th>"
        "<th class='roName' data-sort-type='text'>FA</th>"
        "<th data-sort-type='number'>등급</th>"
        "<th data-sort-type='text'>원 소속</th>"
        "<th data-sort-type='text'>계약 구단</th>"
        "<th class='roEntry' data-sort-type='number'>직전 연봉</th>"
        "<th class='roEntry' data-sort-type='number'>선수+현금</th>"
        "<th class='roEntry' data-sort-type='number'>현금 보상</th>"
        "<th class='roStatus' data-sort-type='text'>상태</th>"
        "<th class='roAction' data-sort-type='text'>처리</th>"
        "</tr></thead><tbody>");

    for (int i = 0; i < record_count && i < 800; i++) {
        KboFaCompensationRecord* rec = &records[i];
        char original_team[16] = "-";
        char signing_team[16] = "-";
        char previous_salary[32] = "-";
        char cash_with_player[32] = "-";
        char cash_only[32] = "-";
        const char* grade_display = kbo_fa_market_display_grade(rec->grade);
        uint32_t grade_sort_rank = kbo_fa_market_display_grade_sort_rank(rec->grade);
        int row_selected = detail_rec != NULL && detail_rec->player_id == rec->player_id;

        kbo_fa_compensation_copy_record_text(
            rec,
            original_team,
            sizeof(original_team),
            signing_team,
            sizeof(signing_team),
            previous_salary,
            sizeof(previous_salary),
            cash_with_player,
            sizeof(cash_with_player),
            cash_only,
            sizeof(cash_only));

        kbo_window_text_appendf(buffer, row_selected ? "<tr class='selected'>" : "<tr>");
        kbo_window_text_appendf(buffer, "<td data-sort-value='%u'>%u</td>", rec->signed_on_yyyymmdd, rec->signed_on_yyyymmdd);
        kbo_webview_append_player_name_cell(buffer, rec->player_name, rec->player_id);
        kbo_window_text_appendf(buffer, "<td data-sort-value='%u'>", grade_sort_rank);
        kbo_html_append_escaped(buffer, grade_display);
        kbo_window_text_appendf(buffer, "</td><td>");
        kbo_html_append_escaped(buffer, original_team);
        kbo_window_text_appendf(buffer, "</td><td>");
        kbo_html_append_escaped(buffer, signing_team);
        kbo_window_text_appendf(buffer, "</td><td class='roEntry' data-sort-value='%d'>", rec->previous_salary);
        kbo_html_append_escaped(buffer, previous_salary);
        kbo_window_text_appendf(buffer, "</td><td class='roEntry' data-sort-value='%u'>", rec->cash_with_player);
        kbo_html_append_escaped(buffer, cash_with_player);
        kbo_window_text_appendf(buffer, "</td><td class='roEntry' data-sort-value='%u'>", rec->cash_only);
        kbo_html_append_escaped(buffer, cash_only);
        kbo_window_text_appendf(buffer, "</td><td class='roStatus'>");
        kbo_html_append_escaped(buffer, kbo_fa_compensation_status_label(rec->status));
        kbo_window_text_appendf(buffer, "</td><td class='roAction'>");
        if (rec->requires_player_compensation && rec->protect_count > 0u) {
            kbo_window_text_appendf(buffer, "<a class='rightsTextAction' title='보상 결정 열기' href='kbo://fa-comp/detail/%u'>열기</a>", rec->player_id);
        } else {
            kbo_html_append_escaped(buffer, "-");
        }
        kbo_window_text_appendf(buffer, "</td></tr>");
    }
    if (record_count == 0) {
        kbo_window_text_appendf(buffer, "<tr><td colspan='10' class='roEmptyMessage'>기록된 KBO FA 보상 의무가 없습니다.</td></tr>");
    } else if (record_count > 800) {
        kbo_window_text_appendf(buffer, "<tr><td colspan='10' class='roEmptyMessage'>출력이 일부만 표시됩니다. 전체 내역은 fa_compensation.csv에서 확인하세요.</td></tr>");
    }
    kbo_window_text_appendf(buffer, "</tbody></table></section>");
}

void kbo_webview_append_fa_compensation_view(
    KboWindowTextBuffer* buffer,
    int selected_compensation_subview,
    uint32_t selected_compensation_player_id)
{
    if (buffer == NULL) {
        return;
    }
    if (selected_compensation_subview < 0
            || selected_compensation_subview >= KBO_HUB_FA_COMP_SUBVIEW_COUNT) {
        selected_compensation_subview = KBO_HUB_FA_COMP_SUBVIEW_TASKS;
    }

    KboFaCompensationRecord* records = (KboFaCompensationRecord*)HeapAlloc(
        GetProcessHeap(),
        HEAP_ZERO_MEMORY,
        (SIZE_T)KBO_FA_COMPENSATION_MAX * sizeof(KboFaCompensationRecord));
    KboFaCompensationProtectionDebugRow* debug_rows = (KboFaCompensationProtectionDebugRow*)HeapAlloc(
        GetProcessHeap(),
        HEAP_ZERO_MEMORY,
        (SIZE_T)KBO_FA_COMPENSATION_PROTECTED_LIST_MAX * sizeof(KboFaCompensationProtectionDebugRow));
    if (records == NULL || debug_rows == NULL) {
        kbo_window_text_appendf(buffer, "<div class='rights rosterRights'><section class='tablewrap rosterTableWrap'>보상 버퍼를 할당하지 못했습니다.</section></div>");
        if (records != NULL) { HeapFree(GetProcessHeap(), 0, records); }
        if (debug_rows != NULL) { HeapFree(GetProcessHeap(), 0, debug_rows); }
        return;
    }

    char path[MAX_PATH] = {0};
    int count = kbo_load_fa_compensation_records(records, KBO_FA_COMPENSATION_MAX, path, sizeof(path));
    int debug_count = kbo_load_fa_compensation_protection_debug_rows(debug_rows, KBO_FA_COMPENSATION_PROTECTED_LIST_MAX);
    KboFaCompensationRecord* detail_rec = kbo_fa_compensation_find_detail_record(
        records,
        count,
        debug_rows,
        debug_count,
        selected_compensation_player_id);

    kbo_window_text_appendf(buffer, "<div class='rights rosterRights faCompensation'>");
    kbo_webview_append_fa_compensation_subtabs(buffer, selected_compensation_subview);

    if (selected_compensation_subview == KBO_HUB_FA_COMP_SUBVIEW_PROTECTION) {
        kbo_webview_append_fa_compensation_protection_view(buffer, records, count, detail_rec);
    } else if (selected_compensation_subview == KBO_HUB_FA_COMP_SUBVIEW_DECISION) {
        kbo_webview_append_fa_compensation_decision_view(buffer, debug_rows, debug_count, detail_rec);
    } else if (selected_compensation_subview == KBO_HUB_FA_COMP_SUBVIEW_HISTORY) {
        kbo_webview_append_fa_compensation_history_view(buffer, records, count, detail_rec);
    } else {
        kbo_webview_append_fa_compensation_tasks_view(buffer, records, count, debug_rows, debug_count, detail_rec);
    }

    kbo_window_text_appendf(buffer, "</div>");
    HeapFree(GetProcessHeap(), 0, debug_rows);
    HeapFree(GetProcessHeap(), 0, records);
}
