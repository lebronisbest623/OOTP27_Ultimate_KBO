...
#define KBO_ALLSTAR_DATE_SERIALIZER_CALLBACK_OFFSET 0x10u
...
static int kbo_allstar_date_slot_looks_like_serializer_callback(uint8_t* league, uint32_t offset, uintptr_t* out_slot, uintptr_t* out_callback)
{
    ...
    if (slot < KBO_RUNTIME_MIN_USER_POINTER
            || !memory_range_readable((void*)(slot + KBO_ALLSTAR_DATE_SERIALIZER_CALLBACK_OFFSET), sizeof(uintptr_t))) {
        return 0;
    }

    uintptr_t callback = *(uintptr_t*)(slot + KBO_ALLSTAR_DATE_SERIALIZER_CALLBACK_OFFSET);
    if (out_callback != NULL) {
