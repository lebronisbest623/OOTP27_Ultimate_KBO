    ...
    KboSecondaryDraftCandidateRow* out)
{
    ...
    out->saved_protected = kbo_secondary_draft_sql_player_protected(
        season,
        candidate->owner_team_id,
        candidate->player_id);
    out->submitted = submitted ? 1 : 0;
    out->already_drafted = kbo_secondary_draft_sql_result_player_exists(season, candidate->player_id);
