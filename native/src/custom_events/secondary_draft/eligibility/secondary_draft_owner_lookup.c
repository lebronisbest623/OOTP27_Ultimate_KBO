    ...
    uint32_t team_id)
{
    ...
    if (org_team_id != 0u && org_team_id != team_id) {
        for (int i = 0; i < entry_count; i++) {
            if (entries[i].team_id == org_team_id) {
                return entries[i].owner_index;
            }
        }
    }
...
{
    if (out_owner_team_id != NULL) { *out_owner_team_id = 0u; }
    if (out_service_team_id != NULL) { *out_service_team_id = 0u; }
    if (out_military_reserved != NULL) { *out_military_reserved = 0; }
    if (out_non_military_loan != NULL) { *out_non_military_loan = 0; }
    ...
    int military_reserved = player[OOTP27_PLAYER_MILITARY_ACTIVE_OFFSET] != 0u
        || (current_team_id != 0u && kbo_team_id_is_military_service_team(current_team_id))
        || (active_team_id != 0u && kbo_team_id_is_military_service_team(active_team_id))
        || (loan_team_id != 0u && kbo_team_id_is_military_service_team(loan_team_id));

    if (military_reserved) {
        uint32_t service_team_id = current_team_id != 0u && kbo_team_id_is_military_service_team(current_team_id)
            ? current_team_id
            : (loan_team_id != 0u && kbo_team_id_is_military_service_team(loan_team_id) ? loan_team_id : active_team_id);
        ...
        if (kbo_military_resolve_original_team(
                ...
                &original_team_id,
                &original_league_id)) {
            int index = kbo_secondary_draft_owner_map_lookup(entries, entry_count, original_team_id);
            if (index >= 0) {
                if (out_owner_team_id != NULL) { *out_owner_team_id = original_team_id; }
                if (out_service_team_id != NULL) { *out_service_team_id = service_team_id; }
                if (out_military_reserved != NULL) { *out_military_reserved = 1; }
                return index;
            }
        }
    }

    int index = kbo_secondary_draft_owner_map_lookup(entries, entry_count, active_team_id);
    if (index < 0) {
        index = kbo_secondary_draft_owner_map_lookup(entries, entry_count, current_team_id);
    }
    if (index < 0) {
        index = kbo_secondary_draft_owner_map_lookup(entries, entry_count, default_team_id);
    }
    if (index >= 0) {
        if (out_owner_team_id != NULL) {
            *out_owner_team_id = active_team_id != 0u ? active_team_id : current_team_id;
        }
        if (out_non_military_loan != NULL && loan_team_id != 0u && !military_reserved) {
            *out_non_military_loan = 1;
        }
    }
    return index;
}
