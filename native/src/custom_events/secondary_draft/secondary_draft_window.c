...
static int kbo_secondary_draft_window_cb(void* user_data, int ncols, char** vals, char** names)
{
    ...
    window.season = kbo_secondary_draft_sql_u32(vals, 0);
    window.league_id = kbo_secondary_draft_sql_u32(vals, 1);
    window.protection_open_yyyymmdd = kbo_secondary_draft_sql_u32(vals, 2);
    window.protection_deadline_yyyymmdd = kbo_secondary_draft_sql_u32(vals, 3);
    window.draft_yyyymmdd = kbo_secondary_draft_sql_u32(vals, 4);
    ...
    const char* source)
{
    ...
    snprintf(
        ...
        "season,league_id,protection_open_yyyymmdd,protection_deadline_yyyymmdd,draft_yyyymmdd,source,updated_at"
...
int kbo_secondary_draft_load_window(uint32_t season, KboSecondaryDraftWindow* out)
{
    ...
    snprintf(
        sql,
        sizeof(sql),
        "SELECT season,league_id,protection_open_yyyymmdd,protection_deadline_yyyymmdd,draft_yyyymmdd "
        "FROM secondary_draft_windows WHERE season=%u LIMIT 1;",
        season);
...
{
    if (out == NULL || draft_yyyymmdd == 0u
            || !kbo_secondary_draft_ensure_schema("secondary_draft_window_load_by_date_schema")) {
        return 0;
    }
    memset(out, 0, sizeof(*out));
    char sql[512] = {0};
    snprintf(
        ...
        sizeof(sql),
        "SELECT season,league_id,protection_open_yyyymmdd,protection_deadline_yyyymmdd,draft_yyyymmdd "
        "FROM secondary_draft_windows WHERE draft_yyyymmdd=%u ORDER BY season DESC LIMIT 1;",
        draft_yyyymmdd);
    KboSecondaryDraftWindowLoadContext ctx = {out, 0};
    if (!kbo_save_state_query(sql, kbo_secondary_draft_window_cb, &ctx, "secondary_draft_window_load_by_date")) {
        return 0;
    }
    return ctx.found;
}
