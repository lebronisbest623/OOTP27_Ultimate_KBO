...
static int kbo_secondary_draft_apply_military_rights_transfer(
    uint8_t* player,
    const KboSecondaryDraftCandidate* candidate,
    const KboSecondaryDraftTeam* to_team,
    uint32_t league_id)
{
    if (player == NULL || candidate == NULL || to_team == NULL || to_team->team_id == 0u) {
        return 0;
    }
    uint32_t service_team_id = candidate->service_team_id != 0u
        ? candidate->service_team_id
    ...
    return active_team_id == to_team->team_id || original_team_id == to_team->team_id;
}
    ...
    KboSecondaryDraftPickRow* out_pick)
{
    ...
    int moved = 0;
    int military_rights_transfer = 0;
    int pre = 0;
    int reg = 0;
    int attach = 0;
    if (candidate->military_reserved) {
        military_rights_transfer = kbo_secondary_draft_apply_military_rights_transfer(
            player,
            candidate,
            to_team,
            league_id);
        moved = military_rights_transfer;
    } else {
        kbo_assign_player_to_team_like_ootp(player, to_team_ptr, league_id, &pre, &reg, &attach);
        moved = *(uint32_t*)(player + OOTP27_PLAYER_CURRENT_TEAM_ID_OFFSET) == to_team->team_id;
    }
    if (!moved) {
        kbo_log_runtimef(
            "KBO secondary draft pick failed reason=assignment_not_applied player=%u from=%u to=%u round=%u pick=%u military=%u",
            candidate->player_id,
            from_team->team_id,
            to_team->team_id,
            round,
            pick_no,
            candidate->military_reserved);
        return 0;
    }
    ...
    out_pick->military_rights_transfer = military_rights_transfer;
    ...
    kbo_log_runtimef(
        "KBO secondary draft pick player=%u name=%s from=%u to=%u round=%u pick=%u value=%d cash=%u cash_applied=%d history=%d military_rights=%d pre=%d reg=%d attach=%d",
        out_pick->player_id,
        ...
        history_inserted,
        out_pick->military_rights_transfer,
        pre,
        reg,
        attach);
    return 1;
